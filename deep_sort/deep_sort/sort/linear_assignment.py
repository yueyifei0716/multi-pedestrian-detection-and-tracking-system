...
def min_cost_matching(
    ...
    # 这几个for循环用于对匹配结果进行筛选，得到匹配和未匹配的结果linear_assignment
...
# -------------------3.1级联匹配（优先考虑年龄较小的跟踪器）----------
#挑选出所有确定框，将年龄小的tracks和未检测的匹配框相匹配
def matching_cascade(
    ...
    # track_indices传入的是状态为confirmed跟踪对象，对检测结果和状态为confirmed跟踪对象进行匹配。
    if track_indices is None:
        track_indices = list(range(len(tracks)))#跟踪对象索引
    if detection_indices is None:
        detection_indices = list(range(len(detections)))#检测对象索引

    # 初始化匹配集matches M ← ∅ 
    # 未匹配检测集unmatched_detections U ← D 
    unmatched_detections = detection_indices#初始化为匹配上的检测框
    matches = []
    ...
    for level in range(cascade_depth):
        ...
        # 步骤6：Select tracks by age 挑选出对应年龄的跟踪器
        track_indices_l = [
            k for k in track_indices
            if tracks[k].time_since_update == 1 + level
        ]#从后往前匹配，首先找到time_since_update为1的也就是上一帧刚更新过的那些跟踪对象，可以理解为先匹配连续轨迹然后匹配有间断但为达到max_age的对象。
        # 如果当前level没有track，继续
        if len(track_indices_l) == 0:  # Nothing to match at this level
            continue
            
        # 步骤7：调用min_cost_matching函数进行匹配，将跟踪框和尚未匹配的检测框进行匹配

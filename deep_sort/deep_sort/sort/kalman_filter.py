...
class KalmanFilter(object):
    ...
    # --------------1.2 update阶段----融合了测量信息并做出校正的后验估计------
    def update(self, mean, covariance, measurement):
        ...
        # 计算卡尔曼增益K，卡尔曼增益用于估计误差的重要程度
        # cholesky矩阵分解加快求解；
        # 公式5的右边有一个S的逆，如果S矩阵很大，S的逆求解消耗时间太大，所以代码中把公式两边同时乘上S，右边的S*S的逆变成了单位矩阵，转化成AX=B形式求解。
        ...
        return new_mean, new_covariance#返回当前时刻的后验估计均值和协方差

    def gating_distance(self, mean, covariance, measurements,
                        only_position=False):
        #   马氏距离通过计算检测框距离预测框有多远的偏差来估计跟踪器状态的不确定性，
        # 如果两个框之间的马氏距离大于马氏距离的阈值，则认为这两个框不可能关联

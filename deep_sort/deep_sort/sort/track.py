...
# 储存轨迹信息，包括轨迹框的位置和速度信息，轨迹框的ID和状态（确定，不确定，删除）
class TrackState:
    """
...
class Track:
    ...
    def __init__(self, mean, covariance, track_id, n_init, max_age,
        ...
        #  state代表框的状态,Tentative: 不确定态,Confirmed: 确定态,Deleted: 删除态
    ...
    def predict(self, kf):
        ...
        """
        self.mean, self.covariance = kf.predict(self.mean, self.covariance)  # 进行卡尔曼滤波得到该对象的预测坐标
        self.age += 1  # 预测的帧数加1,track年龄+1
        self.time_since_update += 1  # 距离上次更新加1，该计数表明track已经连续多少次没有与detection匹配

...
def _nn_euclidean_distance(x, y):
    """ Helper function for nearest neighbor distance metric (Euclidean).欧式距离
...
class NearestNeighborDistanceMetric(object):
    ...
    def __init__(self, metric, matching_threshold, budget=None):
        ...
        if metric == "euclidean":
            self._metric = _nn_euclidean_distance  # 使用最邻近欧式距离
        elif metric == "cosine":
            self._metric = _nn_cosine_distance  # 使用最邻近余弦距离
        else:
            raise ValueError(
                "Invalid metric; must be either 'euclidean' or 'cosine'")
        self.matching_threshold = matching_threshold  # 在级联匹配中函数的调用
    ...
    def distance(self, features, targets):
        # 在匹配阶段，将distance封装为gated_metric,
